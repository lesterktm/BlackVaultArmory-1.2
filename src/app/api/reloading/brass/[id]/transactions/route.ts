...
// POST /api/reloading/brass/[id]/transactions - Post a brass ledger entry (adjusts quantityOnHand)
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const brass = await prisma.brassInventory.findUnique({ where: { id } });
    if (!brass) return NextResponse.json({ error: "Brass inventory entry not found" }, { status: 404 });

    const previousQty = brass.quantityOnHand;
    let newQty: number;
    if (ADD_TYPES.has(type)) {
      ...
      if (newQty < brass.quantityReadyToLoad + brass.quantityInProcess) {
        return NextResponse.json(
          { error: "Cannot reduce quantityOnHand below quantityReadyToLoad + quantityInProcess" },
          { status: 400 }
        );
      }
    ...
    const [updated, transaction] = await prisma.$transaction([
      prisma.brassInventory.update({ where: { id }, data: { quantityOnHand: newQty } }),
      prisma.brassTransaction.create({
        data: {
          brassId: id,
          type,
          quantity,
    ...
    return NextResponse.json({ brass: updated, transaction }, { status: 201 });
  } catch (error) {
    console.error("POST /api/reloading/brass/[id]/transactions error:", error);
    return NextResponse.json({ error: "Failed to create brass transaction" }, { status: 500 });
  }
}
