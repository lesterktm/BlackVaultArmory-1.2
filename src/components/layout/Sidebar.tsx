...
const RELOADING_CHILD_ITEMS = [
  { label: "Powder", href: "/reloading/powder", icon: Flame },
  { label: "Primers", href: "/reloading/primer", icon: Zap },
  { label: "Brass / Cases", href: "/reloading/brass", icon: Layers },
  { label: "Bullets", href: "/reloading/bullet", icon: Crosshair },
  { label: "Load Recipes", href: "/reloading/recipes", icon: BookOpen },
  { label: "Batch Log", href: "/reloading/batches", icon: ClipboardList },
] as const;
...
export function Sidebar({ mobileOnly = false, mobileOpen = false, onMobileClose }: SidebarProps) {
  ...
  const [reloadingOpen, setReloadingOpen] = useState(pathname.startsWith("/reloading"));
  const isReloadingRoute = pathname.startsWith("/reloading");
  const reloadingSectionOpen = isReloadingRoute || reloadingOpen;
  ...
  const navContent = (
    <>
      ...
      <nav className="flex-1 overflow-y-auto overscroll-contain py-3 space-y-0.5 px-2 pb-6">
        ...
        <div>
          <button
            onClick={() => setReloadingOpen((prev) => !prev)}
            className={cn(
              "w-full flex items-center gap-3 px-2.5 py-2 rounded-md text-sm transition-all duration-150 group relative",
              isReloadingRoute ? "bg-[#00C2FF]/10 text-[#00C2FF] border border-[#00C2FF]/20" : "text-vault-text-muted hover:text-vault-text hover:bg-vault-border"
            )}
            title={collapsed ? "Reloading" : undefined}
          >
            {isReloadingRoute && <span className="absolute left-0 top-1/2 -translate-y-1/2 w-0.5 h-5 bg-[#00C2FF] rounded-r-full" />}
            <Flame className={cn("shrink-0 transition-colors", collapsed ? "w-5 h-5" : "w-4 h-4", isReloadingRoute ? "text-[#00C2FF]" : "text-vault-text-faint group-hover:text-vault-text-muted")} />
            {!collapsed && (
              <>
                <span className="min-w-0 text-left">
                  <span className="block font-medium tracking-wide truncate">Reloading</span>
                  <span className="block text-[11px] text-vault-text-faint truncate">Components, recipes & batches</span>
                </span>
                <ChevronDown className={cn("w-4 h-4 ml-auto transition-transform", reloadingSectionOpen ? "rotate-180" : "rotate-0")} />
              </>
            )}
          </button>

          {!collapsed && reloadingSectionOpen && (
            <div className="mt-1 ml-4 border-l border-vault-border pl-2 space-y-0.5">
              {RELOADING_CHILD_ITEMS.map((item) => {
                const Icon = item.icon;
                return (
                  <Link key={item.href} href={item.href} onClick={() => onMobileClose?.()} className="flex items-center gap-2 px-2 py-1.5 rounded-md text-xs text-vault-text-muted hover:text-vault-text hover:bg-vault-border transition-colors">
                    <Icon className="w-3.5 h-3.5 text-vault-text-faint" />
